# Oslonett A/S, Mai 1995 / Odd Einar Aurbakken & Anders Ellefsrud
...
sub do_article {
    &prepare;		# Sett opp directorystrukturen
    &process_message;
    &save_message;
}
...
@mapchar = (
   ...
   0x00,		#    305 197 xc5 aksent macron  (har ikke)
   0x00,		#    306 198 xc6 aksent breve   (har ikke)
   0xc7,		#    307 199 xc7
...
sub dofile {
    ...
    print "Reading $file\n" if $DEBUG;
    $N_linetype = &rstr(3);
    # linetype should be 3 uppercase letters
    if ($N_linetype !~ /[A-Z]{3}/) {
      # Perhaps there is some junk ahead of the real news?
      &readto("\001");        # ^A is start character
      $N_linetype = &rstr(3);
      if ($N_linetype !~ /[A-Z]{3}/) { # Abandon all hope
          close(F);
          return;
      }
    }
