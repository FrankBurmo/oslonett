...
# feltene. Format for datafilen (antar i eksempelet at $fieldsep='"'):
#
# kundenr"reg.dato: yymmdd hh:mm:ss"<feltene angitt i @fields, adskilt med ">
#
# NB! Dette scriptet definerer feltnavn og feltrekkefølge i datafilen!
# Andre scripts og FORMS må forholde seg til innholdet i array'en @fields
#
...
# $counterfile  filnavn til tellerfil
...
# $LOCK_UN      Parameterverdi til flock(): "unlock"
    ...
    @fields = ("Firma", "Kontakt", "Adresse", "Postnr", "Poststed", "Telefon",
	       "Telefax", "Email", "Bekreftet", "Annonsetype", "Spesialpris",
    ...
    $counterfile = '/local/www/ON-kundeteller.txt';
    $datafile = '/local/www/ON-kundedata.txt';
    $errlogfile = '/local/www/ON-kundefeil.txt';
    $timeout = 15;
...
# Datafilen er nå låst, kan oppdatere trygt
    ...
    $record = join($fieldsep, ++$counter, &dato);
    for $f ( @fields ) {
	$record .= $fieldsep . $input{$f};
    }
    print FILE "$record\n";
...
# Returnerer HTML-kode med tilbakemelding om at alt gikk bra
    ...
    &printheader("tilbakemelding");

    print "Vi har registrert følgende informasjon om denne kunden:\n";
    print "<hr noshade size=1>\n<dl>\n";

# Neste linje lagt inn av Steinar

    print "<dt> <b>Web kundennummer</b>\n  <dd> $counter\n";

    for $f ( @fields ) {
	printf "  <dt> <b>%s</b>\n  <dd> %s\n",
	$f, $input{$f}||"[ikke oppgitt]";
    }
    print "</dl>\n";
    &printfooter;
}
