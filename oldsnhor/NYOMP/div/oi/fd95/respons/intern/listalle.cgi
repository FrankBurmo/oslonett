...
eller tilbakemeldinger som er kommentert. Ved å klikke på link\'ene
...
foreach $filename (@file) {
    ...
    printf(qq{%-9s <a href="$redutscript?file=$intdir/$filename">%-54s %-7s %s\n},
	   $dato, $attrib{overskrift}."</a>", (length $attrib{revdato}) ? "Gammel":"  Ny", ( $attrib{anonym} =~ /^ja$/i ) ? 
	   "[anonym]" : $attrib{navn});
    close(FILE);
}
