...
%input = &getinput;
$input{maxcount} = 50 unless $input{maxcount};
...
foreach (keys %input) {
    ...
    ($tmp1 = $_) =~ s/([ +?&])/sprintf("%%25%02X",unpack("c",$1))/ge;
    ($tmp2 = $input{$_}) =~ s/([ +?&])/sprintf("%%25%02X",unpack("c",$1))/ge;
    $forminput .= "$tmp1%3D$tmp2%26";
}
...
tilbake til <a href="form-new-cd.html">søkeskjemaet</a> og fyll ut igjen
...
sub dosearch {
	...
	$pos = $cat{lc join(";", @input{product,category})};
	dbmclose %cat;
	seek(K, $pos, 0);
    }
...
#        }
    ...
    $feil .= "Feil i søkeuttrykk for artist: $originput{artist}<br>\n" if length $@;
    eval($input{title}) if length $input{artist};
    $feil = "Feil i søkeuttrykk for tittel: $originput{title}<br>\n" if length $@;
    &error($feil) if length $feil;

    &printheader("Søkeresultat, $cat", 'kurv');
...
while (<K>) { 
  ...
  $catcutoff
	...
	@f{@FIELDS} = split(/;/, $cand);
	next unless $input{product} =~ /$f{product}/;
	...
	push(@sortkeys, lc join(" ", @f{'product', 'category',
    ...
    foreach (@matches) {
	@f{@FIELDS} = split(/;/);
	if (!length $input{category} && $lastcat ne join(": ", 
							 @f{product,category})) {
	    $lastcat = join(": ", @f{product,category});
	    $list .= qq!<tr>\n<td colspan="3" align="center">!;
	    $list .= qq!<font size="+1">!;
	    $list .= qq!<b><em>$lastcat</em></b></font><br></td>\n!;
	}

	$imgurl = $ikurv{$f{no}} ? $CHECKGIF : $NOCHECKGIF;
	$altimg = $ikurv{$f{no}} ? 'x' : ' ';

	$f{price} =~ s/,(\d\d)\d+/.$1/;
	$f{price} = '[ukjent]' if $f{price} == 0;
	$list .= qq!<tr>\n!;
	$list .= qq!<td valign="middle"> &#160;$f{artist}</td>\n!;
	$list .= qq!<td valign="middle"> &#160;<a href="showitem-new.cgi?no=$f{no}">!;
	$list .= qq!$f{title}</a>$f{recording}</td>\n!;
	$list .= qq!<td valign="middle" align="right" width="80">$f{price}&#160;</td>!;
	$list .= qq!<td valign="middle"><a href="http://www.sn.no/kurv/hent.cgi/akersmic/$f{no}?ref=$ENV{SCRIPT_NAME}%3F$forminput">! .
	    qq!<img alt="$altimg" src="$imgurl" border="0"! .
	    qq!width="20" height="20"></a><br></td>\n!;
    }
...
#print "debug: id=$id<p>\nikurv=<br>", join("<br>\n",keys %ikurv);
	...
	if ($counter > $input{maxcount}) {
	    print "<em>Søket ble <strong>avbrutt</strong> etter $input{maxcount} treff.</em><p>\n";
	} else {
	    print "<em>Tilsammen $counter titler i listen nedenfor.<p></em>\n";
	}
	$leftheader = ($input{product} eq 'SPILL') ? 'Kategori' : 'Artist/gruppe';
	print <<EOT;
<table border="1" cellpadding="0">
<tr>
<td width="80"><font size="+2">&#160;$leftheader</font></td>
...
EOT
    ...
    &log($counter);
print qq!<p>Tilbake til <a href="/uh/am-kat/form-new-cd.html">søkesiden</a>.\n!;
