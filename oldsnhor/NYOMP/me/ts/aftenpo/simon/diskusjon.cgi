...
### NB - IKKE KOPIER DETTE PROGRAM SOM TEMPLAT FOR ANDRE
### DISKUSJONSGRUPPER. DET ER FOR MYE HACKET PÅ. BRUK EN
### AV DE ANDRE INSTANSENE AV diskusjon.cgi [steinar]
...
# Endret av Mariann Ødegård 15. januar 1996
# .. og av Steinar 20.2

# Konfigurering gjøres kun i de neste 7 linjene samt i subrutinene
# 'header' og 'footer'

# pga. Aftenpostens behov måtte vi nok hakke lit mere (SK)

$DEFAULTGRUPPE = "Simon på nettet";

$TOPPDIR	= "/local/www/me/ts/aft/simon";	# top directory
$TOPP		= "/simon";		# top URL
...
$gruppe = $DEFAULTGRUPPE unless length $gruppe;
...
# KG - måtte fjerne header kall - <img ..> i title ble ikke bra! (SK)
#&header(qq!<img alt="På skross med Simon" src="/ungskole/simon/bilder/simon1.gif">!);
# SPytt ut header her
print<<EOT;
...
<html>
<head>
 <title>"På skråss med Simon", torsdag 22. februar 1996, kl. 13-18.00</title>
</head>
<body bgcolor="#ffffee" link="#0000ff" vlink ="#AA0000">
<blockquote><blockquote>
<h1 align="center"><img alt="På skross med Simon" src="/ungskole/simon/bilder/simon1.gif"></h1>
EOT
print ;
print qq!<center>!;
...
sub form {
    &header(qq!Nytt innlegg i diskusjonsgruppen<br> "$gruppe"!);
...
sub submit {
    ...
    &header("Takk for den innsendte artikkelen");
    print <<EOT;

Artikkelen blir synlig når den blir besvart av Simon, så du må kanskje vente
ca. fem minutter.
<p>
Når den blir synlig, får den URL\'en <a
href="$ENV{'SCRIPT_NAME'}/$gruppeurl/$filename">$ENV{'SCRIPT_NAME'}/$gruppeurl/$filename</a>.<p>

Hvis du senere ønsker å slette artikkelen, må du oppgi passordet
"<em>$passwd</em>" og nummeret på artikkelen din: <em>$count</em>. 
Du finner artikkelen du vil slette ved å bruke følgende URL:

<blockquote> <a
href="$ENV{'SCRIPT_NAME'}/$gruppeurl/cancel">http://$ENV{'SERVER_NAME'}$ENV{'SCRIPT_NAME'}/$gruppeurl/cancel</a>
</blockquote>

<font size="+1">Gå tilbake til diskusjonsgruppen <a
href="$ENV{'SCRIPT_NAME'}/$gruppeurl">$gruppe</a></font>
EOT
...
innleggene i gruppen). Bruk kun <i>tallene</i> i URL'en. Videre må du oppgi 
passordet du valgte eller
...
sub showarticle {
    ...
    # Spesialtilpasning for aftenposten: ikke vis artikler før de er
    # besvart og publisert:
    &error("Innlegget $arg finnes men er ikke besvart ennå")
	unless $art{published};
...
sub urlescape {
    ...
    $url =~ s!(["% &?/æøåÆØÅ])!sprintf("%%%02X",unpack("C",$1))!ge;
    return $url;
}
...
sub item {
    ...
    # Spesialtilpasning for Aftenposten (utheving av postinger fra redaksjonen)
    $emph_on = $emph_off = '';
    if ($published{$o} =~ /original/) {
	$emph_on = qq!<em><font size="+0" color="#ff0000">!;
	$emph_off = "</font></em>";
    }

    push(@text,$indent . qq! $on$title{$o}$off, $emph_on$name{$o}$emph_off\n!);
...
# pass 1 read file, build @children lists for all articles
	...
	# Spesialtilpasning for aftenposten: ikke vis artikler før de er
	# besvart og publisert:
	next unless $input{published};

	$count++;
	$dato = $1 if $input{regdato} =~ /(\S+)/;
	$input{'title'} =~ s/<.+?>//g;
	$title{$filename} = $input{'title'};
	$published{$filename} = $input{'published'};
...
# pass 3 traverse @orphans, print reference tree
	...
	$info .= qq!Dessverre ingen innlegg tilgjengelige ennå.\n!;
...
<a href="/info/">[Info]</a>
<a href="/">[Aftenposten hjemmeside]</a>
<a href="/info/post/">[Post]</a>
</font>
<p>
<hr size="1" noshade align="center" width="20%">
<address>
<font size="-1">
Denne tjenesten er laget for 
<a href="http://www.aftenposten.no/">Aftenposten</a>
av <a href="http://www.sn.no/sn/">Schibsted Nett AS</a> (C) 1996.
</font>
</address>
...
<textarea name="body" cols="60" rows="15">XXBODY</textarea>
</td>
