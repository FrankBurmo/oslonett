#!/local/bin/perl
# Script for å laste stikkordlsiter fra fil i argv(1)
#

require "../lib/tittellib.pl";

$dump = shift;
$bak  = $stikkdatafile . ".bak";  #Backup
`cp $datafile $bak`;
...
open(STIKK, ">$stikkdatafile") || die "Kan ikke åpne stikkordsfil $artistdata\n";
...
while (<DMP>) {
    ...
    $nr++;
($stikk,$tit,$artist,$knr,$genre,$mdl,$tid) = split(/%/);
...
@rec = 
    ($nr,"$idag","$idag",$stikk,$tit,$artist,$knr,$genre,$mdl,$tid);
