...
# $pslogofile   fil som inneholder Oslonett/WWW-logo. inkluderes i ps-dokument
...
%fieldname	= ("Tittelnr", "Web tittelnummer",
...
@fields		= ("Tittelnr", "RegDato", "EndreDato", "Label", "Artist",
                   "Tittel", "Genre", 
                   "Kat.nr", "År", "Pris");

@stikkfields	= ("Stikknr", "RegDato", "EndreDato", "Stikkord", "Tittel",
		   "Artist", "Kat.nr", "Genre", "Label", "Tid" );

# (Rekkefølgen av elementene i @fields er vesentlig.
#  Kan derfor ikke bruke keys(%fieldname) for @fields.

@simplefields	= ("Tittel", "Label", "Kat.nr", "År", "Pris" );

@textareas	= ("Diverse");

@skip		= ("mailto", "reply-to", "subject", "cc", "visalle",
		   "sortering", "format", "Nyartist", "Nygenre");
@required	= ("Label", "Artist", "Tittel", "Genre");

$fieldsep	= '"';		# Man mister alle forekomster av ", erstatter
$sepcode        = '\'';		# disse med ' som antas å være likeverdig.

$endrescript	= 'http://www.sn.no/fono/adm/edittitle.cgi';
$finnscript	= 'http://www.sn.no/fono/finntittel.cgi';
$privfinnscript	= 'http://www.sn.no/fono/adm/finntittel.cgi';
$finnstikkscript  = 'http://www.sn.no/fono/finnstikk.cgi';

$basedir	= '/local/www/nl/ndiv/fono';
$counterfile 	= "$basedir/lib/FONO-tittelteller.txt";
$errlogfile 	= "$basedir/lib/FONO-tittelfeil.txt";
$datafile 	= "$basedir/lib/FONO-titteldata.txt";
$stikkdatafile 	= "$basedir/lib/FONO-stikkorddata.txt";

# Husk at neste variabel også står hardkodet i medllib.pl 
# med et annet navn. Verdien skal være den samme

  $mdldata 	= "$basedir/lib/FONO-medlemsdata.txt";

# END husk

$genredata 	= "$basedir/lib/genre.txt";
$artistdata 	= "$basedir/lib/artist.txt";
$nytittelform 	= "$basedir/adm/nytittel.htm";
...
if ($ua eq "Mozilla" && $ver >= 1.1) {$ssl_ok = 1;}
...
# i @felter og values = verdiene fra parameteren.
# @felter overføres by-name
    local($entry) = @_[0];
    local($fieldsep) = @_[1];
    local(*felter) = $_[2];
