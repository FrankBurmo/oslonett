...
#   my $skiplist = 'index\.htm?l|\/ssi|\/forem|\/smorem|css|\/js\/|\/adm\/|\#';
   my $skiplist = '\/ssi|\/gifs|\/img|\/styret\/|css|\/js\/|\#';
   $skiplist .= '|\/meldinger\/' unless $query{topdir} eq 'meldinger';

# 3:
# Start for rekursjonen
 my $top     = "/home/oslonett/historie/OL";
 my $root = $top;
...
$slist = "<p>Ingen filtreff</p>"
 ...
 open (I,"$root/search.tmpl") ||
    error ("Kan ikke åpne malfil for søk");
...
sub show_results {
    ...
    print "Location: http://priv.infostream.no/oslonett/cgi/search.cgi$query{path}\n\n";
    exit;
