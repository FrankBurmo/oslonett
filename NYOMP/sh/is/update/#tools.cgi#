...
#
#
# DOKUMENTASJON: Forventes aa bli lagt opp i lopet av forholdsvis kort tid 
#             - utg. januar, beg. av februar. 
#
#
#             
...
#--------------------------------------------------
# endreavdeling_i_forms
#--------------------------------------------------
sub endreavdeling_i_forms {
	&write_header("Endre en avdeling");
...
<p>
    ...
    # Enten har vi fått koden for hvilken avdeling vi skal endre via et forms,
    # eller så er det lagt med som parameter (ved direktevalg fra liste)
	
    $in{'avdeling'} = $param if $in{'avdeling'} eq "";

    # Hent ut avdelingsbeskrivelsen ved aa soeke gjennom avdelingsdatabasen
    foreach $_ (@AVD) {
...
#--------------------------------------------------
# insert_avd - setter inn en avdeling ved pkt. X
#
#--------------------------------------------------
sub  insert_avd {
	local($avd( = @_;
	...
	# 1. Finnes avdelingen?
	# Ja - flytt alle avd. nedenfor ett hakk ned
	# 2. Insert X som avdeling
	# 3. Recount
...
#--------------------------------------------------
# remove_avd - fjerner avdelingen X og alle underavdelinger.
#              Hvis Y=1 fjerner den ogsaa alle produktene
#              som hoerer under denne kategorien. 
#              Returnerer en liste over alt som ble
#              fjernet.
#--------------------------------------------------
sub remove_avd {
	local($avd, $prod_fjern) = @_;


	# 1. Fjern avdelingen X
	# 2. fjern alle underavelinger
	# 3. Fjern alle tilh;rende produkter
	# 4. Recount
...
sub move_avd {
	...
	# 1. Remove X , men ikke recount
	# 2. Insert Xcontent at Y
	# 3. Recount
...
#--------------------------------------------------
# change_cat - endrer kategori (baade art. og prod.)
#         
# NB! Sjekker ikke for overstrykninger!!!!
#
# EX: change_cat 1.2.3 1.2.4
#
#--------------------------------------------------
sub change_cat {
	local($from, $to) = @_;
...
#--------------------------------------------------
# thislevel - returnerer en liste over alle avd.
# 	      p] DETTE niv[et...
#
#--------------------------------------------------
sub thislevel {
	local($avd) = @_;
	local(@list,$oavd,$count);

	$count=0;
	# Finner "overavdeling" - f.eks. 1.2 hvis avd= 1.2.3, "" hvis avd=3
	$oavd = "";
	$oavd = $1 if $avd=~ /^(.*)\.(\d+)$/;

	# Finn startpunkt, f.eks. 3 hvis avd = 1.2.3, 1 hvis avd = 1.2.1, 2 hvis avd=2
	$start= (2 != "") ? ($2 : $1); 

	# For alle som med denne overkategori som start, lagre i en liste
	foreach (@AVD) {
		$tmp = $_ if /^$oavd\.(\d+)\./;
		@list[$count++] = $_ if ($1 >= $start);
	}
...
#--------------------------------------------------
#
# upcount - returnerer avdelingen OVER den som
#           sendes med som parameter.
# EX: &upcount 1.3 vil gi 1.2
#     &upcount 1.3.1 vil gi 0
#     &upcount 1.3.2 vil gi 1.3.1
#--------------------------------------------------
sub upcount {
	local($_) = @_;
	
	# Returnerer 0 hvis det ikke finnes noen "tidligere" avdeling
	return 0 if /\.1$/;
	return s/\.(d+)$/"." . ($1-1)/e;
}
...
#--------------------------------------------------
#
# downcount -returnerer avdelingen UNDER den som sendes
# 	med som parameter
#
# EX: &downcount 1.3 vil gi 1.4
#     &downcount 1.3.1 vil gi 1.3.2
#--------------------------------------------------
sub downcount {
	local($_) = @_;
	return s/\.(d+)$/"." . ($1+1)/e;
