...
#Fyll inn her med passende standard-bruker av systemet
$DEFAULTSENDER = "Simon";
...
<center>
<em>
<h4>Innsender er ansvarlig for innholdet i innleggene<br>
Brukerene kan ikke se artikler som er markert med "ennå ikke publisert"<br>
For å se gruppen fra brukernes grensesnitt, bruk 
<a href="http://www.aftenposten.no/simon/diskusjon.cgi/$gruppeurl">http://www.aftenposten.no/simon/diskusjon.cgi/$gruppeurl</a>
</h4>
</em>
</center>
EOT
...
sub submit {
    ...
    $input{'from'} .= " ($input{'epost'})" if length $input{'epost'};
    delete($input{'name'});
    delete($input{'epost'});
    ...
    $input{'published'} = 'yes (original posting from redaksjon.cgi)';
    ...
    &header("Ny artikkel mottatt (ref: $input{ref})");
...
<blockquote>
<a href="$ENV{'SCRIPT_NAME'}/$gruppeurl/cancel">http://$ENV{'SERVER_NAME'}$ENV{'SCRIPT_NAME'}/$gruppeurl/cancel</a>
</blockquote>

Denne artikkelen har nå blitt publisert slik at den er synlig for
brukere av det åpne grensesnittet mot diskusjonsgruppen. Samtidig er
eventuell referert artikkel også publisert.<p>

<font size="+2">Gå tilbake til diskusjonsgruppen <a
...
sub showarticle {
    ...
    $ikkebesvart=qq!<form method="GET" action="http://www.aftenposten.no/simon/diskusjon-adm/publisere.cgi"><input type="hidden" name="arg" value="$gruppe/$arg">
<b><font size="+1" color="#ff0000">Innlegget er ikke publisert ennå</font>.
<font size="+1" color="#000000">
<p><input type="submit" value="Publiser dette nå"></font></b></form>! unless $art{published};
    print <<EOT;
<h2>$art{'title'}</h2>
Innsender: <b>$art{'from'}</b><br>
Dato: <b>$art{'date'}</b><br>
$ikkebesvart<p>
$art{'body'}
...
sub item {
	...
	if ($published{$o} =~ /original/) {
	    $emph_on = qq!<em><font size="+0" color="#006000">!;
	    $emph_off = "</font></em>";
	}
    } else {    
	$ikkebesvart = qq!<font size="+0" color="#ff0000"><b>(Ikke besvart ennå)</b></font>!;
    }
    push(@text, $indent . qq! $on$title{$o}$off, $emph_on$name{$o}$emph_off $ikkebesvart\n!);
...
<body bgcolor="#dddddd" link="#0000ff" vlink ="#AA0000">
...
<center>
  <font size="-1">
Denne tjenesten er laget for <a href="http://www.aftenposten.no/">Aftenposten</a>
...
<td><input name="name" size="45" value="XXSENDER"><br></td>
