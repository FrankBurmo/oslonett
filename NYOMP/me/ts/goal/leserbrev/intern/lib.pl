#!/local/bin/perl -w
#
# Definerer felles variable og rutiner for GOALs leserbrev-scripts
#
...
$topdir		= "/home/frogner/www/me/ts/goal/leserbrev";
$intdir		= "$topdir/intern";
$tellerfil	= "$intdir/brev-teller.txt";

$topurl		= "/me/ts/goal/leserbrev";
$utscript	= "$topurl/ut.cgi";
$redutscript	= "$topurl/intern/redut.cgi";
...
#	$value =~ s/(\n$quotemark)/<br>$1/g if $name =~ /^leserbrev$|^svar$/i;
#	$value =~ s/\n\s*\n+/<p> /g if $name =~ /^leserbrev$/i;

	$value =~ s/\n/<br>\n/g if $name =~ /^leserbrev$|^svar$/i;
...
<body bgcolor="#e0e0e0" text="#000000" alink="#ff8410" vlink="#c00000" link="#ff0000">
<hr><center><img alt="" src="../gifs/goal_200.gif" width=150></center><hr>
<h1>$title</h1>
</center>
@;
...
<address>
  Tilrettelagt for <a href="/me/ts/goal/">GOAL</a> av <a href="/">Oslonett AS</a>.
</address>
...
# forsøket på å flock'e ble gitt opp.
    ...
    print &header("Tellerfilen er opptatt");
    print <<EOT;
<h2>Tellerfilen er låst av en annen prosess, forsøk igjen litt senere.</h2>
