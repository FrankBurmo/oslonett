...
# lag en full utskrift for hvert firma
          $line = "<font size=+1><b>$old{'Firma'}</b></font>\n
          <blockquote>\n";
...
# Ønsker riktig sortering av ÆØÅ.
    ...
    $overskr .= "Telefon  " if $input{'felt.tel'};
    $overskr .= "  Adresse                                            "
        if $input{'felt.adr'};
...
\\section*{Liste over FONO medlemmer}
\\small
$overskr
\\hline
@list
...
sub showfirm {
	...
	if ($old{URL}) {
	    $tmp = "<a href=\"$old{URL}\">$old{'Firma'}</a>\n";
	} else {
	    $tmp = "$old{'Firma'}\n";
    };
    ...
    print "<dd>v. $old{'Kontakt'}<br>\n" if $old{'Kontakt'};
    print<<EOT;
	<dd>$old{Adresse}<br>
	<dd>$old{Postnr} $old{Poststed}<br>
EOT
	if ($old{Email}) {
	    print "Email: <a href=\"mailto:$old{Email}\">$old{'Email'}</a>\n";
	};
    print "<p>$old{'Diverse'}<br>\n" if $old{'Diverse'};
# Space
    $firm = $old{Firma};
    $firm =~ s/\ /\+/g;    
#    $firm =~ s/\/ /%2F/g;    
    print<<EOT;
       <p> <a href="finntittel.cgi?format=html&Label=$firm&felt.artist=on&felt.genre=on">Liste over utgivelser</a>
    </blockquote>
EOT
    &printfooter;
