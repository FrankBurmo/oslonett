#!/local/bin/perl
# Script for å gjøre oppdateringer i FOM medlemsbasen
#
...
# utfylt, returnerer feilmelding og avslutter hvis >= 1 felt mangler.
	...
	&printheader("Oppdatering av medlemsdata: for få felter utfylt");
	print <<EOT;
...
# Datafilen er nå låst, kan oppdatere trygt
	...
	push(@kunder, $_);
	$kundeindeks = $#kunder
	    if ( (split($fieldsep, $_))[0] == $input{'Medlemsnr'});
    }
...
# slik at gjenopprettet post kan settes inn i posisjon $kundeindeks
	...
	if ($kundeindeks == @kunder) {
	    &printheader("Ikke-eksisterende medlemsnummer");
	    print <<EOT;
Medlemsnummeret ($input{'Medlemsnr'}) som ønskes slettet
...
EOT
	...
	&printheader("Tilbakemelding: har slettet medlemsdata");
	print <<EOT;
Data om "$input{'Firma'}" (medlem nr $input{'Medlemsnr'}) er nå slettet fra
medlemsdatabasen. <p>

Dersom medlemmet ble slettet ved et uhell, kan data gjenopprettes ved å
...
EOT
	...
	if (@lines) {
	    ...
	    &printheader("Tilbakemelding: har oppdatert medlemsdata");
	    if ($kundeindeks == @kunder) {
		print <<EOT;
<blockquote>
<hr noshade size=2>
Data om dette medlemmet er tidligere slettet men blir nå gjenopprettet.
<hr noshade size=2>
</blockquote>';
EOT
            }
	    ...
	    print <<EOT;
Følgende nye data er lagt inn for medlemmet "$input{'Firma'}" (medlemsnr. $input{'Medlemsnr'}):<p>
<dl>
@lines
</dl>
EOT
	    ...
	    print "Ingen nye data er lagret for medlemmet ";
	    print "\"$input{'Firma'}\" (medlemsnr. $input{'Medlemsnr'})<p>\n";
	}
    }
    &printfooter;
