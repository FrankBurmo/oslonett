...
sub dosearch {
    $ret =  eval("/$input{'Firma'}/"); # ønsker å trap'e evt. feil i regexp
    if ( ! defined($ret) ) {
...
EOT
	...
	for $name ( 'Firma', 'Annonsetype', 'Kontakt1', 'Kontakt2') {
	    $input{$name} = '.*' unless $input{$name};
	}

	push(@matches, $firm) 
	    if ( 
		(!$input{'Kundenr'} || $old{'Kundenr'} == $input{'Kundenr'}) &&
		$old{'Firma'} =~ /$input{'Firma'}/i &&
		$old{'Annonsetype'} =~ /^$input{'Annonsetype'}$/ &&
 		$old{'Kontakt1'} =~ /^$input{'Kontakt1'}$/ &&
		$old{'Kontakt2'} =~ /^$input{'Kontakt2'}$/ );
    }
    @matches;
}
...
sub showlist {
	...
	next unless $input{'visgamle'} eq 'ja' || $old{'Bekreftet'} !~ /gammel/i;
	next unless $input{'visikkebekreftet'} eq 'ja' || $old{'Bekreftet'} =~ /ja/i;
        next if ($old{'Annonsetype'} eq 'Stillingsannonse' || 
                 $old{'Annonsetype'} eq 'Konsulentoppdrag' );
	$old{'Firma'} =~ s/^\s+//;	
	...
	if ($input{'format'} ne 'html') {
	    ...
	    $line = sprintf("%s& %s", $latexfirm, $old{'Kundenr'});
		...
		if $input{'felt.adr'};
	    $line .= sprintf("& %s", $old{'Annonsetype'})
		if $input{'felt.type'};
	    if ($input{'felt.pris'} && 
		($old{'Annonsetype'} ne "Stillingsannonse"
		 || $input{'Annonsetype'} eq "Stillingsannonse")) {
		$line .= sprintf("& %s& ", $price{$old{'Annonsetype'}});
...
# HTML
	    $line = sprintf("<a href=%s?Kundenr=%d>%-50s%4d  ",
			    $finnscript, $old{'Kundenr'},
			    $old{'Firma'}."</a>", $old{'Kundenr'});
		...
		if ($old{'Annonsetype'} ne "Stillingsannonse"
                    && $old{'Annonsetype'} ne "Konsulentavtale"){
                    if ($old{'Spesialpris'}) 
                    {
                      $line .= sprintf(" %8s ", $old{'Spesialpris'});
		    } else {
		...
		if $input{'felt.kontakt2'};
	    $line .= "\n";
            if ($input{'verbose'} eq "on") {
		$line .= "</pre><blockquote><font size=-1>$old{'Adresse'}<br>\n";
                $line .= "$old{'Postnr'} $old{'Poststed'}<br>\n";
                $line .= "attn. $old{'Kontakt'}<br>\n" 
                    if $old{'Kontakt'};
                $line .= "<p>$old{'Diverse'}\n"
                    if $old{'Diverse'};
                $line .= "<p>Kundekontakt hos SN: $old{'Kontakt1'}<br>\n";
                $line .= "Registreringsdato: $old{'RegDato'}\n";
                $line .= "</font></blockquote><pre>\n";
	    }
	}
	push(@list, $line);
...
# Ønsker riktig sortering av ÆØÅ.
	...
	push(@no, $old{'Kundenr'}); # alfabetisk eller numerisk sortering hhv.
    }
...
if ($input{'format'} eq 'html') {
    &printheader("Liste over kunder med annonsepakke: <em>$old{'Annonsetype'}</em>");
    print "<pre>\n$overskr\n\n@list\n</pre>\n";

    print "Tilsammen $totalcount kunder i listen ovenfor.<p>\n\n";
    
    print "Listen inkluderer også kunder som er markert som «tidligere kunde».<p>\n\n" if ($input{'visgamle'} eq "ja");
    print "Listen inkluderer også kunder som er ikke er bekreftet.<p>\n\n" if ($input{'visikkebekreftet'} eq "ja");
    
    print "Faste leieinntekter, pr. kvartal (spesialavtaler ikke medregnet): NOK $sumprice<p>\n\n" if $input{'felt.pris'} && $sumprice;
...
sub showfirm {
    ...
    &printheader("Oppdatering av data i kundedatabasen");
